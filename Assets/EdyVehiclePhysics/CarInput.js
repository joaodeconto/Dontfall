...
function Start()
	{
	m_CarTelemetry = GetComponent(CarTelemetry);	
...
function Update ()
	...
	if (Input.GetKeyDown(KeyCode.R) && bShift)
		{
		var DeformScript = m_Car.GetComponent(CarDamage);
...
function DisableCar(Car : CarControl)
	...
	var CarCams : CarCameras = Car.GetComponent(CarCameras);
...
function SelectCar(Car : CarControl)
	...
	m_Car = Car;	
	m_CarSettings = m_Car.GetComponent(CarSettings);
	m_CarCameras = m_Car.GetComponent(CarCameras);
...
//		m_bAccelRelease = true;
	...
	if (m_bBrakeRelease)
		...
		if (speedSideways < GEARSPEEDMIN && speedRotation < GEARSPEEDMIN) // && m_bAccelRelease)
			{
...
//	if (motorInput > 0) m_bAccelRelease = false;
...
function OnGUI ()
    ...
    if (GUI.Toggle(Rect(rc.x+160, rc.y+2, 50, 18), true, "car >", guiSkin.button) == false ||
		GUI.Button(Rect(rc.x+220, rc.y, 400, 20), m_CarSettings.description, "helptext"))
		SwitchCar(1);
	...
	if (showHelp)
		{
		if (GUI.Button(rc, "Offroader v4 / 2011.01.02 - by Edy", guiSkin.box))
			showHelp = false;
