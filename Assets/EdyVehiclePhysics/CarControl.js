...
function Start ()
	...
	var WheelC : WheelCollider = WheelFL.GetComponent(WheelCollider);
...
function FixedUpdate ()
	...
	// autoSteerMax determina el ángulo en el que las ruedas ofrecen la máxima fuerza de giro (peakSlip) a la velocidad actual.
	...
	if (autoSteerLevel > 0.0) 
		...
		if (forwardSpeed > peakSlip)
			angleMax = 90.0 - Mathf.Acos(peakSlip / forwardSpeed) * Mathf.Rad2Deg;
		else
			angleMax = steerMax;
		}
	else 
	...
	// -------- Giro de dirección ruedas delanteras (con ajuste del mesh)
	...
	if (steerInput > 0.0)			// Giro a la derecha
		...
		if (m_steerR > angleMax) m_steerR = angleMax;
		...
		if (m_steerL < -angleMax) m_steerL = -angleMax;
...
function ApplyTraction(Wheel : CarWheel, motorSlip, brakeSlip, handBrakeInput : float)
	{
